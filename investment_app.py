...
# --- 스타일 커스터마이징 (CSS 주입) ---
# 1. 폰트: Pretendard (가독성 좋은 현대적 고딕체)
# 2. 테마: 다크 모드 (블랙/다크그레이 배경 + 네온 컬러 포인트)
st.markdown("""
    <style>
        ...
        /* 메트릭(숫자) 스타일 - 금융 터미널 느낌 */
        [data-testid="stMetricValue"] {
            font-family: 'Pretendard', monospace !important;
            font-weight: 700 !important;
            color: #00FF9D !important; /* 형광 녹색 포인트 */
        }
        
        /* 카드형 컨테이너 스타일링 (Streamlit 컨테이너 경계선) */
        [data-testid="column"] {
...
st.markdown("""
<div style='background-color: #1E2329; padding: 15px; border-radius: 5px; border-left: 5px solid #00FF9D;'>
    <strong>철학적 엔진(Engine)과 실전 시뮬레이션(Simulation)의 결합</strong><br>
    이 모델은 <strong>단기 전술(20%)</strong>과 <strong>중기 전략(80%)</strong>을 분리하여 운용하며, 
...
with col1:
    ...
    st.markdown("<p style='font-size: 0.8em; color: #888;'>Engine: 래리 윌리엄스 (변동성 돌파)</p>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div style="background-color: rgba(255,255,255,0.05); padding: 10px; border-radius: 5px; border-left: 4px solid {color_map.get(tac_color, '#A0A0A0')}; margin-bottom: 10px;">
            <span style="color: white; font-weight: bold; font-size: 1.1em;">{tac_signal}</span>
        </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown("<p style='font-size: 0.8em; color: #888;'>Engine: 소로스 & 하워드 막스 (역발상)</p>", unsafe_allow_html=True)
    
    st.markdown(f"""
        <div style="background-color: rgba(255,255,255,0.05); padding: 10px; border-radius: 5px; border-left: 4px solid {color_map.get(str_color, '#A0A0A0')}; margin-bottom: 10px;">
            <span style="color: white; font-weight: bold; font-size: 1.1em;">{str_stance}</span>
        </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    fig.update_layout(
        ...
        height=500,
        paper_bgcolor='rgba(0,0,0,0)', # 배경 투명하게 (앱 배경색 사용)
